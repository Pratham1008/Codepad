...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto w-full flex flex-col h-full overflow-y-auto">
      ...
      {snippets.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 text-on-surface-variant bg-surface-container border border-outline-variant rounded-xl border-dashed">
          <FileCode2 size={48} className="mb-4 opacity-50" />
          <p className="font-semibold">No snippets yet.</p>
          <p className="text-sm">Head over to the playground to write and save some code.</p>
        </div>
      ) : (
            ...
            <Link key={snippet.snippetId} href={`/editor/${snippet.snippetId}`}>
              <div className="bg-surface border border-outline-variant rounded-xl p-5 hover:border-primary transition-all group flex flex-col h-full cursor-pointer hover:shadow-md">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="bg-surface-variant text-on-surface-variant text-xs font-code-sm px-2 py-1 rounded">
                    {snippet.language}
                  </div>
                </div>
