...
export function SettingsClient({ initialPasskeys, userProfile: initialUserProfile, initialError }: SettingsClientProps) {
  const safeInitialPasskeys = Array.isArray(initialPasskeys) ? initialPasskeys : [];
  const [passkeys, setPasskeys] = useState<any[]>(safeInitialPasskeys);
  ...
  const reloadData = async () => {
    setLoading(true);
    const [pkRes, profileRes] = await Promise.all([getPasskeys(), getUserProfile()]);
    if (!pkRes?.error) {
      setPasskeys(Array.isArray(pkRes) ? pkRes : []);
    }
  ...
  return (
    <div className="w-full h-full overflow-y-auto">
      ...
      <section className="bg-surface border border-outline-variant rounded-xl p-6 mb-8">
        ...
        ) : (passkeys && Array.isArray(passkeys) && passkeys.length > 0) ? (
            ...
            {passkeys.map(pk => (
              <li key={pk.id} className="flex justify-between items-center p-3 bg-surface-container rounded border border-outline-variant">
                <div>
                  ...
                  <div className="text-xs text-on-surface-variant mt-0.5" suppressHydrationWarning>
